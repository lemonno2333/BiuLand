...
enum PickupBrandCatalog {
    ...
    nonisolated static let brands: [PickupBrand] = [
        ...
        PickupBrand(name: "汉堡王", aliases: ["BURGER KING"], category: .food, keywords: ["取餐号", "汉堡", "皇堡"], iconName: "fork.knife"),
        PickupBrand(name: "塔斯汀", aliases: ["TASTIEN"], category: .food, keywords: ["取餐号", "汉堡", "中国汉堡"], iconName: "fork.knife"),
        PickupBrand(name: "华莱士", aliases: ["WALLACE"], category: .food, keywords: ["取餐号", "炸鸡", "汉堡"], iconName: "fork.knife"),
        PickupBrand(name: "老乡鸡", aliases: [], category: .food, keywords: ["取餐号", "米饭", "快餐"], iconName: "fork.knife"),

        PickupBrand(name: "顺丰", aliases: ["顺丰速运", "SF EXPRESS"], category: .express, keywords: ["快递", "包裹", "取件码"], iconName: "shippingbox.fill"),
    ...
    nonisolated static func fallbackCategory(for text: String) -> PickupCategory {
        let normalizedText = normalize(text)
        if ["取件", "快递", "包裹", "驿站", "菜鸟", "丰巢", "开柜", "柜机", "提货", "取货"].contains(where: { normalizedText.contains(normalize($0)) }) {
            return .express
        }
        if ["饮品", "取茶", "茶号", "奶茶", "咖啡", "拿铁", "美式", "冰饮", "热饮"].contains(where: { normalizedText.contains(normalize($0)) }) {
            return .drink
        }
        return .food
    }
